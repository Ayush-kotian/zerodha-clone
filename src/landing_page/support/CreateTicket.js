import React from "react";
function CreateTicket() {
  return (
    <div className="container mt-5">
      <div className="row text-center">
        <h1 className="mt-5 fs-3" style={{ textAlign: "justify" }}>
          To create a ticket, select a relevant topic
        </h1>
        <div className="col-4">
          <h4 className="fs-5 mt-5" style={{ textAlign: "justify" }}>
            <i class="fa-solid fa-circle-plus"></i> Account opening
          </h4>
          <div style={{ textAlign: "justify" }} className="mt-4">
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Online Account Opening
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Offline Account Opening
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Company, Partnership and HUF Account
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Opening
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              NRI Account Opening
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Charges at Zerodha
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Zerodha IDFC FIRST Bank 3-in-1 Account
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Getting Started
            </a>
          </div>
        </div>
        <div className="col-4">
          <h4 className="fs-5 mt-5" style={{ textAlign: "justify" }}>
            <i class="fa-solid fa-user"></i> Your Zerodha Account
          </h4>
          <div style={{ textAlign: "justify" }} className="mt-4">
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Login Credentials
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Account modification and Segment Addition
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Dp ID and back details
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Your Profile
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Transfer and conversion of shares
            </a>
          </div>
        </div>
        <div className="col-4">
          <h4 className="fs-5 mt-5" style={{ textAlign: "justify" }}>
            <i class="fa-solid fa-chart-column"></i>
            {"  "}Your zerodha Account
          </h4>
          <div style={{ textAlign: "justify" }} className="mt-4">
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Margin/leverage,Product and Order types
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Kite Web and Mobile
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Trading FAQs
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Corporate Actions
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Sentinel
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Kite API
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Pi and other platforms
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              stock reports +
            </a>
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              GTT
            </a>
          </div>
        </div>
        <div className="col-4 mt-5">
          <h4 className="fs-5 mt-5" style={{ textAlign: "justify" }}>
            <i class="fa-regular fa-credit-card "></i> Funds
          </h4>
          <div style={{ textAlign: "justify" }} className="mt-4">
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Adding Funds
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Fund Withdrawal
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              eMandates
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Adding Bank Accounts
            </a>
            <br />
          </div>
        </div>
        <div className="col-4 mt-5 mb-4">
          <h4 className="fs-5 mt-5" style={{ textAlign: "justify" }}>
            <i class="fa-solid fa-circle-notch"></i> Console
          </h4>
          <div style={{ textAlign: "justify" }} className="mt-4">
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Reports
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Ledger
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Portfolio
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              60 Day Challenge
            </a>
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              IPO
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Referal Program
            </a>
            <br />
          </div>
        </div>
        <div className="col-4 mt-5 mb-4">
          <h4 className="fs-5 mt-5" style={{ textAlign: "justify" }}>
            <i class="fa-regular fa-circle fa-sm"></i> Coin
          </h4>
          <div style={{ textAlign: "justify" }} className="mt-4">
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Understanding Mutual Fur
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              About Coin
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Buying and Selling things
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Starting an SIP
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Managing yout Portfolio
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Coin App
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Moving to Coin
            </a>
            <br />
            <a href="" style={{ textDecoration: "none", lineHeight: "2.5" }}>
              Government Securities
            </a>
            <br />
          </div>
        </div>
      </div>
    </div>
  );
}

export default CreateTicket;
